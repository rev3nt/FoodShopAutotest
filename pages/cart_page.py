...
class CartPage(BasePage):
    ...
    def get_cart_summ(self):
        # Находим элемент на странице, получаем текст
    ...
    def get_all_product_cards(self):
        product_cards_locator = (
            By.XPATH,
            "//div[@class='store-card-container d-grid m-2']//div["
            "contains(@class, 'store-card border rounded-1 overflow-hidden my-1')]"
        )
    ...
    def get_product_info_from_card(self, product_card):
        ...
        product_price = product_card.find_element(
            By.XPATH,
            ".//div[@class='fs-5 align-content-center m-1 text-nowrap']"
        ).text
        amount_input = product_card.find_element(
            By.XPATH,
